...
class Run:
    def __init__(self, station_list, train_list, task: str):  # конструктор создания списка станций, поездов , и заданий
        self.station_list = station_list
        self.train_list = train_list
        self.task = task

    def handler(self):    # функция для обработки запросов командной строки
        ...
        for i in range(0, len(list)):
            ...
            if list[i].lower() == 'myself':
                myself = True
                day = int(list[-1])
        if start and not myself:  # условие для создания вручную
            while True:
                ...
                if event.lower() == 'погрузка':
                    ...
                    break
        elif start and myself:    # Для автоматической работы
            run = Run(self.station_list, self.train_list, self.task)
            a = [1, 2, 3, 2]
            task = []
